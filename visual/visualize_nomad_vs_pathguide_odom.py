...
"""visualize_nomad_vs_pathguide.py
...
Added overlays (per your request):
  - NoMaD (left): a *white dashed* line highlights the finally selected candidate
    (nearest-neighbor match to /waypoint_nomad).
  - PathGuide (right): a *black solid* line highlights the finally selected candidate
    (nearest-neighbor match to /waypoint).

Optional (disabled by default):
  - --draw-odom: draw /odom trajectory over the last --odom-sec seconds.
"""
...
class Saver(Node):
    def __init__(self, args):
        ...
        # Odom buffer: stores recent poses for drawing actual path
        # items: (t_ros, x, y, qx, qy, qz, qw)
        self.odom_buf = deque(maxlen=int(self.args.odom_maxlen))
        ...
        self.create_subscription(Odometry, self.args.odom_topic, self.cb_odom, arr_qos)
    ...
    def cb_odom(self, msg: Odometry):
        try:
            ...
            self.odom_buf.append((t, float(p.x), float(p.y), float(q.x), float(q.y), float(q.z), float(q.w)))

            # prune by time window
            if self.args.odom_sec is not None and float(self.args.odom_sec) > 0.0:
                t_min = t - float(self.args.odom_sec)
                while self.odom_buf and self.odom_buf[0][0] < t_min:
                    self.odom_buf.popleft()
        except Exception:
            # odom is optional; never crash the visualizer
            return
    ...
    def _yaw_from_quat(qx: float, qy: float, qz: float, qw: float) -> float:
        """Yaw from quaternion (x,y,z,w), ROS ENU convention."""
        # yaw (z-axis rotation)
        s = 2.0 * (qw * qz + qx * qy)
        c = 1.0 - 2.0 * (qy * qy + qz * qz)
        return float(math.atan2(s, c))

    def _odom_rel_xy_in_base(self) -> Optional[np.ndarray]:
        """Return Nx2 odom positions expressed in current base frame (x forward, y left)."""
        if not self.odom_buf:
            return None

        # use latest pose as current base
        t0, x0, y0, qx0, qy0, qz0, qw0 = self.odom_buf[-1]
        yaw0 = self._yaw_from_quat(qx0, qy0, qz0, qw0)
        cy = math.cos(-yaw0)
        sy = math.sin(-yaw0)

        pts = []
        for (_t, xi, yi, _qx, _qy, _qz, _qw) in self.odom_buf:
            dx = float(xi - x0)
            dy = float(yi - y0)
            # rotate world->base by -yaw0
            xb = cy * dx - sy * dy
            yb = sy * dx + cy * dy
            pts.append((xb, yb))
    ...
    def _pick_selected_idx(self, trajs: np.ndarray, wp_xy: np.ndarray) -> Optional[int]:
        """Pick which sample index is closest to the published waypoint."""
        if trajs is None or wp_xy is None:
            return None
        if trajs.ndim != 3 or trajs.shape[0] <= 0 or trajs.shape[1] <= 0:
            return None

        widx = int(self.args.waypoint_idx)
        widx = max(0, min(widx, int(trajs.shape[1]) - 1))
        wps = trajs[:, widx, :]  # [S,2]
        d2 = np.sum((wps - wp_xy.reshape(1, 2)) ** 2, axis=1)
        if d2.size == 0:
            return None
    ...
    def tick(self):
        ...
        # Odom (actual path) in current base frame (optional)
        odom_xy = self._odom_rel_xy_in_base() if bool(getattr(self.args, "draw_odom", False)) else None
        ...
        if odom_xy is not None:
            uo, vo = self.to_pixels(odom_xy, ih, iw)
            self.axL.plot(uo, vo, linewidth=3.0, alpha=0.95, color="black", linestyle="-")
        ...
        if nomad is not None:
            ...
            # White line: selected trajectory (match to /waypoint_nomad)
            if seq in self.wp_nomad:
                sel = self._pick_selected_idx(nomad[:S], self.wp_nomad[seq].wp)
                if sel is not None:
                    u, v = self.to_pixels(nomad[sel], ih, iw)
                    # highlight selected NoMaD trajectory: white dashed
                    self.axL.plot(u, v, linewidth=4.0, alpha=0.98, color="white", linestyle="--")
        ...
        if odom_xy is not None:
            uo, vo = self.to_pixels(odom_xy, ih, iw)
            self.axR.plot(uo, vo, linewidth=3.0, alpha=0.95, color="black", linestyle="-")

        for i in range(S):
            u, v = self.to_pixels(pg[i], ih, iw)
            self.axR.plot(u, v, linewidth=2.5, alpha=0.95, color=colors[i], linestyle="-")

        # Black line: selected trajectory (match to /waypoint)
        if seq in self.wp_final:
            sel = self._pick_selected_idx(pg[:S], self.wp_final[seq].wp)
            if sel is not None:
                u, v = self.to_pixels(pg[sel], ih, iw)
                self.axR.plot(u, v, linewidth=4.0, alpha=0.98, color="black", linestyle="-")
...
def main():
    ...
    ap.add_argument("--waypoint-idx", type=int, default=2, help="Index along the trajectory used for nearest-neighbor matching to /waypoint(/waypoint_nomad).")

    # Odom overlay
    ap.add_argument("--odom-topic", default="/odom")
    ap.add_argument("--odom-sec", type=float, default=5.0, help="Draw /odom trajectory within last N seconds.")
    ap.add_argument("--odom-maxlen", type=int, default=5000, help="Ring buffer size for odom messages.")
    ap.add_argument("--draw-odom", action="store_true", help="If set, draw /odom trajectory (default: off).")
