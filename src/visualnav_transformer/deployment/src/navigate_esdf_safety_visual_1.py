    ...
    while rclpy.ok():
        ...
        if len(context_queue) > model_params["context_size"]:
            ...
            with torch.no_grad():
                ...
                # ------------------------------------------------------
                # --- sampling noise (for executed policy) ---
                # Draw the noise ONCE per planning step. This keeps the executed behavior identical
                # regardless of whether we also compute raw NoMaD candidates for visualization.
                noisy_action = torch.randn((args.num_samples, model_params["len_traj_pred"], 2), device=device)

                # PathGuide-guided candidates (used for control)
                naction = _denoise(noisy_action.clone(), obs_cond, guide_enabled=bool(args.guide))

                # --- optional: publish trajectories + raw NoMaD (no guidance) for visualization ---
                compare_on = bool(getattr(args, "compare_nomad", True))
                if compare_on:
                    # (1) publish PathGuide-constrained candidates
                    na_pg_np = to_numpy(get_action(naction))  # [N, T, 2] in local frame
                    if getattr(args, 'normalize', False):
                        na_pg_np = na_pg_np * max_linvel

                    msg_pg = Float32MultiArray()
                    msg_pg.layout.dim = [MultiArrayDimension(label=f"seq:{plan_step}", size=0, stride=0)]
                    msg_pg.data = na_pg_np.reshape(-1).tolist()
                    node.sampled_actions_pg_pub.publish(msg_pg)

                    # (2) compute/publish raw NoMaD candidates without affecting control
                    ...
                    if getattr(args, "compare_sync", False):
                        # Debug-only: synchronous raw NoMaD (may reduce control rate)
                        naction_nomad = _denoise(noisy_action.clone(), obs_cond, guide_enabled=False)
                        na_nomad_np = to_numpy(get_action(naction_nomad))
                        if getattr(args, 'normalize', False):
                            na_nomad_np = na_nomad_np * max_linvel

                        msg_nomad = Float32MultiArray()
                        msg_nomad.layout.dim = [MultiArrayDimension(label=f"seq:{plan_step}", size=0, stride=0)]
                        msg_nomad.data = na_nomad_np.reshape(-1).tolist()
                        node.sampled_actions_nomad_pub.publish(msg_nomad)

                        # Also publish the raw NoMaD chosen waypoint (red star) using the same sample index (0)
                        wp_nomad = na_nomad_np[0, -1] if na_nomad_np.ndim == 3 else np.array([0.0, 0.0], dtype=np.float32)
                        wp_nomad_msg = Float32MultiArray()
                        wp_nomad_msg.layout.dim = [MultiArrayDimension(label=f"seq:{plan_step}", size=0, stride=0)]
                        wp_nomad_msg.data = [float(wp_nomad[0]), float(wp_nomad[1])]
                        node.waypoint_nomad_pub.publish(wp_nomad_msg)
                    else:
                        if do_submit:
                            # Asynchronous raw NoMaD (recommended; does NOT change RNG / control outputs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--normalize",
        action="store_true",
        help="Scale (de-normalize) sampled trajectories by max_linvel before publishing /sampled_actions_* (useful for meters-based visualization).",
    )
    ...
    # ---------------- visualization / ablation ----------------
    # In this *_visual script, publishing the "raw NoMaD" (no guidance) vs "PathGuide" candidates
    # is enabled by default so the saver script can always draw dashed vs solid trajectories.
    _cmp = parser.add_mutually_exclusive_group()
    _cmp.add_argument(
        "--compare-nomad",
        dest="compare_nomad",
        action="store_true",
        help="(Default: ON) Publish /sampled_actions_nomad (raw NoMaD) + /sampled_actions_pg (PathGuide) + /waypoint_nomad for visualization.",
    )
    _cmp.add_argument(
        "--no-compare-nomad",
        dest="compare_nomad",
        action="store_false",
        help="Disable publishing the visualization topics: /sampled_actions_nomad, /sampled_actions_pg, /waypoint_nomad.",
    )
    parser.set_defaults(compare_nomad=True)

    parser.add_argument(
        "--compare-seed",
        type=int,
        default=0,
        help="[DEPRECATED] Kept for backward-compatibility. Raw-NoMaD candidates are computed from the SAME noise tensor as the executed policy, so this flag is ignored.",
    )
    parser.add_argument(
        "--compare-every",
        type=int,
        default=10,
        help="When compare is on, compute/publish raw NoMaD once every N planning steps (default: 10). Use 1 to publish every step (heavier).",
    )
    parser.add_argument(
        "--compare-sync",
        action="store_true",
