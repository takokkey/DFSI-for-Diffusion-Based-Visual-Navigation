...
class ViT(nn.Module):
    ...
    def forward(
        ...
        assert (
            x.shape[2] == self.image_height
        ), f"input image height is not {self.image_height}"
        assert x.shape[3] == self.image_width * (
            self.context_size + 2
        ), f"input image width is not {self.image_width}*(context_size + 2)"
...
class MaskedGoalViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        assert (
            self.w % (context_size + 2) == 0
        ), "context_size must be a factor of numbers of patches in width"
        ...
        self.all_cross_masks = torch.stack(
            [self.no_cross_mask, self.goal_cross_mask], dim=0
        )
    ...
    def forward(self, img, input_goal_mask=None):
        ...
        final_mask = torch.index_select(
            self.all_cross_masks.to(device), 0, input_goal_mask.to(device)
        )
        ...
        final_mask = torch.index_select(
            self.mean_mask.to(device), 0, input_goal_mask.to(device)
        ).unsqueeze(-1)
        x = x * final_mask
        x = x.mean(dim=1)
