...
def main(args):
    ...
    # Enable it explicitly with --safety-enable (and you can force-off with --safety-disable).
        ...
        if len(context_queue) > model_params["context_size"]:
            ...
            with torch.no_grad():
                ...
                for step_idx, k in enumerate(noise_scheduler.timesteps[:]):
                    ...
                    # 2) PathGuide TSDF/ESDF 梯度注入（结合 FSM 状态）
                    if args.guide:
                        # 通过 /map_fsm_state 读取当前地图状态
                        with _state_lock:
                            fsm_state = _latest_map_state

                        # 根据 FSM 状态自适应调整几何引导强度：
                        #   S0: 正常 grad_scale
                        #   S1: 半强度 grad_scale
                        #   S2: 禁用几何梯度（仅保留视觉策略）
                        if fsm_state == "S2":
...
if __name__ == "__main__":
    ...
    parser.add_argument("--safety-enable", action="store_true", help="(Optional) Enable ESDF hard safety filter (default: disabled)")
